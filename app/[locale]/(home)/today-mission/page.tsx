...
export default function Component() {
  ...
  const [todayMission, setTodayMission] = useState<number | null>(null);
  const [answer, setAnswer] = useState<string>('');
  const [message, setMessage] = useState<string>(''); // 메시지 상태 추가
  const userId = useUserStore((state) => state.userInfo?.userId);
  const missionChoice = async()=> {
    try{
      const response = await apiClient.get(`/reward/mission/today`);
  ...
  const fetchMission = async()=> {
    ...
    try{
      const response = await apiClient.get(`/reward/mission/${todayMission}`);
      const missionData = response.data ? response.data : null;
  ...
  const missionAnswer = async(e: React.FormEvent<HTMLFormElement>)=> {
    ...
    try{
      const response = await apiClient.post(`/reward/mission/success/${todayMission}`,
        {
          userId: userId,
  ...
  useEffect(() => {
    missionChoice(); // 오늘의 미션 ID 가져오기
  }, []);

  useEffect(() => {
    if (todayMission !== null) {
      fetchMission(); // todayMission이 설정된 후에만 세부 정보를 가져옴
    }
  }, [todayMission]);
