...
export default function SignInPage() {
  ...
  const handleSubmit = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    try{
      // 백엔드 API 호출
      const response = await apiClient.post('/api/v1/user/sales/join', data);
      router.push(`/${locale}/sales/login`);
    }catch(error){
      ...
      <section className="flex flex-col items-center w-full h-auto bg-green-600 md:flex-row">
        ...
        <div className="flex items-center justify-center w-full h-full px-6 bg-white md:w-1/2 xl:w-1/3 lg:px-16 xl:px-12">
          <div className="w-full max-w-md">
            ...
            <p className="mt-8">
              이미 가입되어 있나요?
              <Link href={`/${locale}/sales/login`} className="ml-2 font-semibold text-green-500 hover:text-green-700">
                로그인하기
              </Link>
            </p>
